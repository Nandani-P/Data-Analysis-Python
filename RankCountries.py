...
class readCriteria():
    ...
    def criteria(self):
        ...
        for i, j in countryAgeSeries.itertuples(index=False):
            ...
            diCountries.update({i:addCountry})            
        ...
        for i, j in CountryIncSeries.itertuples(index=False):            
            if i in diCountries:
                existingCountryObj = diCountries[i]
        ...
        for index, row in dfDemoScore.iterrows():
            existingCountryObj = diCountries[row['country']]         
        ...
        cnt = 1     
        for country in (sorted(diCountries.values(), key=operator.attrgetter('popDensity'))):           
        ...
        with open('rankfile.csv', 'w', newline='', encoding='utf8') as f:
            ...
            for country in (sorted(diCountries.values(), key=operator.attrgetter('aggrRank'))):
